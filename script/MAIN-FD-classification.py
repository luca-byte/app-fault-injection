...
def main(name, dataset, dnn, layer, shape, feat_ex, ext_clf, clazz, n=None):
    if clazz==-1:
        ...
        wdir = f"{name}/L{layer}/class_{clazz}"

    log_file_path = f"{wdir}/log/logging.log"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fault Injection Classification")
    parser.add_argument(
        "--pair",
        type=str,
        required=True,
    ...
    parser.add_argument("--layer", type=int, default=0, help="Layer to be used")
