...
log = f"CIFAR-MN-all/L{layer}-{index}/log/LeNet.log"
wdir = f"CIFAR-MN-all/L{layer}-{index}"
seed = None
device = torch.device("cuda")
dataset = torchvision.datasets.CIFAR10(
    "~/dataset/cifar", transform=transformer, download=True, train=False
)
model_path = "CIFAR-MN-all/MobileNetV2_CIFAR10.pth"
...
block = layer
pre_path = "CIFAR-MN-all/CIFAR-MN-all.json"

feat_ex = Preprocessing.load(pre_path, use_scaler=True)

ext_clf = ModelTrainer.load("CIFAR-MN-all/CIFAR-MN-all.pth", 330, 10)
