...
log = f"FMNIST-all/L{layer}-{index}/log/LeNet.log"
wdir = f"FMNIST-all/L{layer}-{index}"
seed = None
device = torch.device("cuda")
dataset = torchvision.datasets.FashionMNIST(
    "~/dataset/fmnist", transform=transformer, download=True, train=False
)
model_path = "FMNIST-all/LeNet_FashionMNIST.pth"
...
block = 0
pre_path = "FMNIST-all/FMNIST-all.json"

feat_ex = Preprocessing.load(pre_path, use_scaler=True)

ext_clf = ModelTrainer.load("FMNIST-all/FMNIST-all.pth", 125, 10)
