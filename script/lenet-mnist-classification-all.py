...
log = f"MINST-all/L{layer}-{index}/log/LeNet.log"
wdir = f"MNIST-all/L{layer}-{index}"
seed = None
device = torch.device("cuda")
dataset = torchvision.datasets.MNIST(
    "~/dataset/mnist", transform=transformer, download=True
)
model_path = "MNIST-all/LeNet-MNIST.pth"
...
pre_path = "MNIST-all/MNIST-2.json"

feat_ex = Preprocessing.load(pre_path, use_scaler=True)

ext_clf = ModelTrainer.load("MNIST-all/MNIST-1.pth", 122, 10)
