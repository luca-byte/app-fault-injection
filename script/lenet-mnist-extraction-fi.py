...
log = f"FSIM_W_FE/L{layer}-{index}/log/LeNet.log"
wdir = f"FSIM_W_FE/L{layer}-{index}"
seed = None
device = torch.device("cuda")
dataset = torchvision.datasets.MNIST(
    "~/dataset/mnist", transform=transformer, download=True
)
model_path = "FSIM_W_FE/LeNet-MNIST.pth"
...
feat_ex = Preprocessing.load(pre_path, use_scaler=False)
...
def main():
    ...
    for fault, k in fi_setup.iter_fault_list():
        ...
        try:
            ...
            df_stats = df_stats[~df_stats.apply(tuple, 1).isin(golden_df_stats.apply(tuple, 1))]
