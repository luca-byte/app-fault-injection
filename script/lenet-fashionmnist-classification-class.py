...
@torch.no_grad()
def main(class_chosen):
    wdir = f"FMNIST-cls/L{layer}-{class_chosen}"
    ...
    class_zero_indices = [i for i, (_, target) in enumerate(test_data_loader.dataset) if target == class_chosen]
